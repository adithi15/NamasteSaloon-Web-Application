...
//     window.open(
//       getWhatsAppUrl(
//         `Hi! I'd like to book an appointment with ${healer.name}.`,
//       ),
//       "_blank",
//       "noopener,noreferrer",
//     );
...
//         {/* Specialists Grid Layout: sExact Match to Dental Mockup */}
//         <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-8 max-w-4xl mx-auto">
//           {" "}
//           {SPECIALISTS.map((healer) => (
//             <div
//               key={healer.id}
//               //colour card
...
export default function SpecialistSection({ onSelectSpecialist }) {
  ...
  return (
    // 1. CHANGED: Added bg-[#F4EEE0] to the section background here
    <section className="relative py-20 md:py-28 bg-[#F4EEE0]" id="specialists-section">
      {/* Adjusted ambient glows slightly so they still look natural on the new background color */}
      <div className="absolute top-1/2 right-10 w-[500px] h-[500px] bg-[#2D5446]/8 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-10 left-10 w-[400px] h-[400px] bg-[#E9DFC9]/40 rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {SPECIALISTS.map((healer) => (
            <div
              key={healer.id}
              // 2. CHANGED: Card background is now a crisp, semi-translucent white (bg-white/80) 
              // and the border color has been softened to seamlessly blend over the cream backdrop.
              className="bg-white/80 backdrop-blur-sm border border-[#DECBA5]/30 p-5 rounded-3xl shadow-sm hover:scale-[1.01] transition-all duration-300 group flex flex-col justify-between"
              id={`healer-card-${healer.id}`}
            >
              <div>
                {/* Media container */}
              ...
              <div className="mt-6 pt-4 border-t border-black/5">
                <button
                  onClick={() => handleBook(healer)}
                  className="w-full py-3 bg-[#1E3E34] hover:bg-[#2D5446] text-white font-bold rounded-xl text-xs uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer group/btn"
                >
                  <span>Book Appointment</span>
