...
export default function MembershipsSection({ onSelectPlan, preview = false, onViewAll }: MembershipsSectionProps) {
  ...
  // In preview mode, show only a handful of cards (prefer popular ones first)
  const displayedPlans = preview
    ? [...filteredPlans].sort((a, b) => (b.popular ? 1 : 0) - (a.popular ? 1 : 0)).slice(0, 3)
    : filteredPlans;
  ...
  return (
    <section className="relative py-20 px-4 sm:px-6 lg:px-8 bg-transparent" id="memberships-section">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {/* Filter Selection Pills — hidden in preview mode */}
        {!preview && (
        ...
        {/* "See More" CTA — only in preview mode */}
        {preview && (
          <div className="flex justify-center mt-14">
            <button
              onClick={onViewAll}
              className="px-8 py-3.5 bg-transparent border border-[#1E3E34] text-[#1E3E34] hover:bg-[#1E3E34] hover:text-white text-xs uppercase tracking-widest font-extrabold transition-all duration-300 flex items-center gap-2 rounded-xl cursor-pointer hover:scale-105 active:scale-95"
            >
