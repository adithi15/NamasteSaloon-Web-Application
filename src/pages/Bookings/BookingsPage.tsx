    ...
    >
      <div className="space-y-6 sm:space-y-8">
        ...
        {bookingsList.length === 0 ? (
          <div className="text-center py-12 sm:py-16 bg-white/40 backdrop-blur-sm border border-[#DECBA5]/30 p-6 sm:p-8 rounded-2xl sm:rounded-3xl space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {bookingsList.map((book) => (
              <div
                key={book.id}
                className="bg-white/60 backdrop-blur-sm border border-[#DECBA5]/30 p-4 sm:p-6 rounded-2xl flex flex-col justify-between hover:border-[#DECBA5]/50 transition-colors shadow-sm"
              >
                <div className="space-y-3.5">
                  <div className="flex justify-between items-start gap-2">
                    <span className="text-[9px] uppercase font-mono tracking-wider font-extrabold text-[#DECBA5] bg-[#DECBA5]/10 border border-[#DECBA5]/20 px-2.5 py-1 rounded shrink-0">
                      Confirmed Session
                    </span>
                    <span className="text-[10px] font-mono text-slate-400 font-bold break-all text-right min-w-0">
                      ID: {book.id}
                    </span>
                  </div>
                ...
                <div className="pt-4 mt-4 border-t border-black/5 flex flex-wrap gap-3">
                  <button
